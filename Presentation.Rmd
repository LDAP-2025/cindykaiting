...
H3: Relationship satisfaction predicts separation/ divorce. 
* We initially wanted to include divorce outcomes but there was no data collected, so we will relate our findings to divorce outcomes reported externally. 
...
# A lot of missing data, future direction is multiple imputations.. 
...
## More complex models: 
We decided to focus on financial stress, and explore a more complex model that includes social support as a mediator.
...
## Compare if constrain the loadings of the latent variables (social support): 
```{r, c1_mdl1_lc}
#Social support as a mediator between financial stress and relationship satisfaction
model11_lc <- '
# latent factor for social support: 
...
## Constraining AR: want to do so to test whether the stability of the constructs are consistent over time. 
```{r, c1_mdl1_lc_4} 
model11AR <- '
# latent factor for social support: 
  CW1_SUPPORT =~ l1*CW1_SOCPROV_1 + l2*CW1_SOCPROV_2 + l3*CW1_SOCPROV_3_R
...
## Conclusion 
This longitudinal structural equation modeling study explored how financial stress impacts relationship satisfaction across three waves, with social support as a potential mediator. Further exploration on 